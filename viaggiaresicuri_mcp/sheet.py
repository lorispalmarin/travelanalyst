...
# Se il nodo di dettaglio è vuoto si usa il riassunto di primo piano 
...
def topic_map(sheet: CountrySheet) -> dict[str, Topic]:
    ...
    raw = {
        f"{section_name}.{field}": getattr(nodes, field).model_copy(
            update={"key": f"{section_name}.{field}"}
        )
        for section_name in SECTION_FIELDS
        for nodes in [getattr(sheet, section_name).nodes]
